...
| `assets[].bridge_decimals` | the decimals every transfer of the asset is denominated in on the wire, identical on every gate and the Solana program (see `docs/architecture.md` §2.3). Unset ⇒ the minimum decimals read on-chain across the asset's deployments and its Solana mint — the only safe maximum. Pinning a lower value is allowed; a higher one is refused. Registered with `setBridgeDecimals` before any corridor, write-once, and after `seal()` it is a governance action like a corridor. `send` amounts must be whole multiples of `10^(tokenDecimals − bridge_decimals)` |
...
| `sig_store.tokens` | scoped credentials, one per role: `validator`, `keeper`, `reader`, `admin`, `indexer`. With none set the store refuses to start (it fails closed). `generate_if_unset` mints a random one per role per run into `run_dir/tokens.env`. `indexer` is the Solana relayer's **marker observer** credential — its reports are authoritative (see below), so it goes only to the relayers that `deliver` |
...
| `price_keeper` | the swap pools' **oracle**, for static prices. Every pool refuses a price older than its max age (a day by default), so pools with nothing running this stop quoting every non-stable token a day after their last `setPrice`. Generates `price-keeper` (EVM, `signer` = the pools' `oracle` key) and `solana-price-keeper` (`solana_oracle_keypair` = the Solana pool's oracle). Each token is held at a target: `chain_prices["<chain_id>"][SYM]`, else `prices[SYM]`, else (Solana) `solana.swap.tokens[].price`; the deploy script writes `chain_prices` from each pool's listing. It re-asserts a price `refresh_margin_secs` (default 21600) before it would expire, and walks a changed target in capped steps without breaking the pool's cooldown. It discovers nothing — a token with a real market needs a real feed. `poll_interval_secs` defaults to 600 |
| `indexer` | history + refund eligibility sweep; with the Solana observer (below) the only writer of the lifecycle. EVM chains only — it speaks EVM JSON-RPC, so it never sees a `Claimed` on the Solana gate. That leg is covered by the **marker observer** in the delivering Solana relayer, which reports what it reads on the gate through the store's `Indexer`-scoped routes; without it a transfer delivered on Solana stays `signed`, is flagged `stuck` / `refund_status: eligible` by the sweep, and never leaves the claim and refund queues |
...
| `relayers[].deliver` | run the claim-submitting half. `payer_keypair` pays fees and rent and carries no bridge authority — the validator signatures do. A delivering relayer also runs the **marker observer**: the generated TOML gets `[store] indexer_token_env = "SIG_STORE_INDEXER_TOKEN"`, and the observer reads the gate's `["executed", id]` / `["refunded", id]` PDAs at `finalized` for every transfer the store still lists as pending on this chain, reporting each observed `claimed` / `cancelled` / `refunded` to the store once. It is the Solana gate's stand-in for the EVM `indexer`; it runs only when that token resolves, and the store accepts its reports from that scope alone — the keeper's token stays advisory (M-1) |
