  ...
  for idx in $(j '[.solana.relayers[] | select(.enabled != false)] | to_entries[].key'); do
    ...
    cfg="$CFG_DIR/solana-relayer-$rname.toml"
    {
      ...
      if [[ "$deliver" == "true" ]]; then
        ...
        # It reads the gate's ["executed"|"refunded", id] PDAs and reports them
        # to the store as OBSERVED claimed/cancelled/refunded — the authoritative
        # lifecycle write — on the Indexer-scoped token, and runs only when this
...
# as another.
# INDEXER is the fifth: the Solana relayer's marker observer reports observed
