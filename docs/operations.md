...
- **Every gate and the Solana program must register the same value per asset.** `deploy-from-json.sh` does this (`registering <SYM> bridge decimals`), before corridors and before `seal()`. Since H-2 the scale is inside the submissionId, so a mismatch no longer pays out wrong — it makes the corridor *unsettleable*: every transfer into it strands and has to be walked back through cancel → refund, and `claim` reverts `BridgeScaleMismatch`. Still check `bridgeDecimalsOf(token)` on each gate and `gate-admin show` on Solana when adding a chain; the validators will also refuse to sign across a mismatch and say so in one warning per corridor.
...
- **Give every chain at least two `rpcs`.** Since H-4 the validator corroborates each
  scan window against a second endpoint before signing, and that check is automatic
  whenever two or more are configured — with one endpoint there is nothing to compare
